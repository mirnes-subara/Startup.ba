...
{
    public class LandlordAnalyticsService : ILandlordAnalyticsService
    {
        private readonly eRentDbContext _context;

        public LandlordAnalyticsService(eRentDbContext context)
        {
            _context = context;
        }

        public async Task<LandlordAnalyticsResponse> GetLandlordAnalyticsAsync(int landlordId)
        {
            var response = new LandlordAnalyticsResponse();
            ...
            // Get all rents for landlord's properties with related data
            ...
            var reviews = await _context.ReviewRents
                ...
                .Where(rr => rr.IsActive && propertyIds.Contains(rr.Rent.PropertyId))
                .ToListAsync();
            ...
            var activeProperties = landlordProperties.Where(p => p.IsActive).ToList();
            ...
            // Property Metrics
            response.TotalProperties = landlordProperties.Count;
            response.ActiveProperties = landlordProperties.Count(p => p.IsActive);
            response.InactiveProperties = landlordProperties.Count(p => !p.IsActive);

            // Properties by Type
            response.PropertiesByType = landlordProperties
            ...
            // Properties by City
            response.PropertiesByCity = landlordProperties
            ...
            response.AveragePriceByPropertyType = landlordProperties
                .GroupBy(p => p.PropertyType?.Name ?? "Unknown")
            ...
            {
                Month = month.ToString("yyyy-MM"),
                NewProperties = landlordProperties.Count(p => p.CreatedAt.Year == month.Year && p.CreatedAt.Month == month.Month),
                TotalProperties = landlordProperties.Count(p => p.CreatedAt <= month.AddMonths(1).AddDays(-1))
            }).ToList();
