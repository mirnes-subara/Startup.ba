...
class AmenityEditScreen extends StatefulWidget {
  final Amenity? amenity;

  const AmenityEditScreen({super.key, this.amenity});

  @override
  State<AmenityEditScreen> createState() => _AmenityEditScreenState();
}

class _AmenityEditScreenState extends State<AmenityEditScreen> {
  ...
  void initState() {
    ...
    _initialValue = {
      ...
      "isActive": widget.amenity?.isActive ?? true,
    };
  }
  ...
  Widget build(BuildContext context) {
    return MasterScreen(
      title: widget.amenity != null ? "Edit Amenity" : "Add Amenity",
      showBackButton: true,
      child: _buildForm(),
    );
  }
                  ...
                  if (formKey.currentState?.validate() ?? false) {
                    ...
                    try {
                      if (widget.amenity == null) {
                        await amenityProvider.insert(request);
                        if (mounted) {
                        ...
                        await amenityProvider.update(widget.amenity!.id, request);
                        if (mounted) {
                          ScaffoldMessenger.of(context).showSnackBar(
                            SnackBar(
                              content: const Row(
                      ...
                      if (mounted) {
                        Navigator.of(context).pushReplacement(
                          MaterialPageRoute(
                            builder: (context) => const AmenityListScreen(),
                            settings: const RouteSettings(name: 'AmenityListScreen'),
                          ),
                        );
                      }
                    } catch (e) {
        ...
        children: [
          ...
          Expanded(
            child: Column(
              ...
              children: [
                Text(
                  widget.amenity != null ? 'Edit Amenity' : 'Add New Amenity',
        ...
        child: Column(
          ...
          children: [
            ...
            customSwitchField(
              name: "isActive",
              label: "Active Status",
              initialValue: widget.amenity?.isActive ?? true,
